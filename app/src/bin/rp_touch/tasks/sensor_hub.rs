...
async fn recover_with_cooldown(bus: &'static SharedI2c1Bus) {
    bus.recover(crate::I2cIrqs).await;
    Timer::after(Duration::from_millis(RECOVERY_COOLDOWN_MS)).await;
}

type ImuDevice = qmi8658_driver::Qmi8658<'static, i2c_bus::RetryingDevice<'static, embassy_rp::i2c::I2c<'static, embassy_rp::peripherals::I2C1, embassy_rp::i2c::Async>>>;
type TouchDevice = ft3168_driver::Ft3168<'static, i2c_bus::RetryingDevice<'static, embassy_rp::i2c::I2c<'static, embassy_rp::peripherals::I2C1, embassy_rp::i2c::Async>>>;

#[embassy_executor::task]
pub async fn sensor_hub_task(
    mut imu: ImuDevice,
    imu_pipeline: &'static qmi8658_driver::ImuPipeline,
    mut touch: TouchDevice,
    touch_pipeline: &'static ft3168_driver::TouchPipeline,
    bus: &'static SharedI2c1Bus,
) -> ! {
    loop {
        ...
        if let Err(err) = imu.init().await {
            ...
            recover_with_cooldown(bus).await;
            continue;
        }
                ...
                match imu.read_accel_gyro_raw().await {
                    ...
                    Err(_) => {
                        ...
                        if imu_read_errors >= IMU_READ_ERROR_LIMIT {
                            imu_pipeline
                                .set_state(qmi8658_driver::CaptureState::InitFailed);
                            should_recover = true;
                        }
                    }
                }
            ...
            if touch_ready {
                if now >= next_touch_poll_at {
                    ...
                    match touch.read_touch_sample().await {
                        ...
                        Err(_) => {
                            ...
                            touch_pipeline
                                .push_sample(ft3168_driver::TouchSample::default());
                            if touch_read_errors >= TOUCH_READ_ERROR_LIMIT {
                                touch_ready = false;
                                touch_read_errors = 0;
                                touch_pipeline
                                    .set_state(ft3168_driver::CaptureState::InitFailed);
                                next_touch_reinit_at = Instant::now()
                                    + Duration::from_millis(TOUCH_REINIT_INTERVAL_MS);
                            }
                        }
                    }
                }
                ...
                match touch.init().await {
                    Ok(chip_id) => {
                        touch_pipeline.set_chip_id(chip_id);
                        touch_pipeline
                            .set_state(ft3168_driver::CaptureState::Running);
                        touch_ready = true;
                        touch_read_errors = 0;
